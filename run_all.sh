...
srun --time=1-00:00:00 --gres=gpu:1 --cpus-per-gpu 10 --constraint volta32gb --partition learnlab python script_generation.py --methods disk --add_missing_data True --imputation_method MissForest --datasets ${dataset} --out_path results/disk_${dataset}.txt &> results/disk_${dataset}.log &
done
